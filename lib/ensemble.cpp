...
std::tuple<vec2d, vec3d> SPEnsemble::get_moments(param& params, vecd moments, int N_realizations) {
    ...
    return get_moments(funcs, params, moments, N_realizations);
}


std::tuple<vec2d, vec3d> SPEnsemble::get_moments(vec_d_vecd_f& functions, param& params, vecd moments, int N_realizations) {
        ...
        for (int m=0; m<moments.size(); m++)
            for(int t=0; t<traj_length; t++)
                for (int i=0; i<functions.size(); i++) {
                    moments_traj[m][t][i] += pow( functions[i](state_traj[t]), moments[m] ) / (double)N_realizations;
                }
    }
...
void SPEnsemble::print_moments(vec_d_vecd_f& functions, param& params, vecd moments, int N_realizations, std::string dir) {
    std::tuple<vec2d, vec3d> output = get_moments(functions, params, moments, N_realizations);
    print_output(output, moments, dir);
}
...
void SPEnsemble::print_output(std::tuple<vec2d, vec3d> output, vecd moments, std::string dir){
    for (int m=0; m<moments.size(); m++) {
        char buffer [30];
